    ...
    function renderAnalysisData(data) {
        ...
        contentArea.innerHTML = `
            ...
            <div class="row g-4 mb-4">
                <div class="col-lg-4">
                    <div class="card bg-surface-card p-4 text-center h-100 d-flex flex-column justify-content-center shadow-md">
                        <h6 class="text-white text-uppercase fw-bold mb-3" style="font-size: 0.85rem; letter-spacing: 0.05em;">Career Readiness Index</h6>
                        <div class="position-relative mx-auto" style="width: 180px; height: 140px;">
                            ...
                            <div class="position-absolute top-50 start-50 translate-middle mt-2 text-center">
                                <span class="display-6 fw-bold text-white" id="caScoreNum">${data.readiness_score}</span>
                                <small class="d-block text-secondary" style="font-size: 0.80rem;">out of 100</small>
                            </div>
                        </div>
                        <div class="mt-3">
                            <span class="badge ${data.readiness_score >= 75 ? 'badge-emerald-subtle' : data.readiness_score >= 50 ? 'badge-cyan-subtle' : 'badge-amber-subtle'} py-1.5 px-3 fs-6 fw-bold">
                ...
                <div class="col-lg-8">
                    <div class="card bg-surface-card p-4 h-100 shadow-md">
                        <div class="d-flex align-items-center mb-3">
                            <div class="brand-icon me-3" style="width: 44px; height: 44px;"><i class="bi bi-robot fs-5"></i></div>
                            <div>
                                <h5 class="text-white fw-bold mb-0">AI Diagnostic Summary</h5>
                                <small class="text-secondary">Generated by Gemini AI &bull; ${data.created_at || 'Recent'}</small>
                            </div>
                        </div>
                        <p class="text-secondary mb-4 leading-relaxed fs-6">${data.ai_summary || 'Your profile shows strong foundational momentum.'}</p>
                        
                        <div class="row g-3">
                            <div class="col-sm-6">
                                <div class="p-3.5 rounded-3 bg-surface-elevated border border-subtle">
                                    <h6 class="text-emerald fw-bold mb-2 small"><i class="bi bi-shield-check me-2"></i>Core Strengths</h6>
                                    <ul class="list-unstyled mb-0 small text-secondary">
                                        ${(data.strengths || []).map(s => `<li class="mb-1.5"><i class="bi bi-check2 text-success me-2 fw-bold"></i>${s}</li>`).join('')}
                                    </ul>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="p-3.5 rounded-3 bg-surface-elevated border border-subtle">
                                    ...
                                    <ul class="list-unstyled mb-0 small text-secondary">
                                        ${(data.weaknesses || []).map(w => `<li class="mb-1.5"><i class="bi bi-arrow-right text-warning me-2 fw-bold"></i>${w}</li>`).join('')}
                                    </ul>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-lg-6">
                    <div class="card bg-surface-card p-4 h-100 shadow-md">
                        <h5 class="text-white fw-bold mb-3"><i class="bi bi-radar me-2 text-primary"></i>Competency Benchmark Radar</h5>
                ...
                <div class="col-lg-6">
                    <div class="card bg-surface-card p-4 h-100 shadow-md">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="text-white fw-bold mb-0"><i class="bi bi-bullseye me-2 text-danger"></i>Detected Skill Gaps</h5>
                            <button class="btn btn-sm btn-glass fw-bold" id="deepRoleGapBtn"><i class="bi bi-search me-1"></i>Role Deep Dive</button>
                        </div>
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-4">
                            ${(data.skill_gaps || []).map(gap => `
                                <a href="/recommendations?skills=${encodeURIComponent(gap)}" class="badge bg-danger bg-opacity-20 border border-danger border-opacity-40 text-white p-2 small text-decoration-none fw-semibold" title="Find opportunities requiring ${gap}">
                                    <i class="bi bi-search me-1 text-danger"></i>${gap}
                                </a>
                            `).join('')}
                        </div>

                        <h6 class="text-white text-uppercase fw-bold mb-2 small" style="letter-spacing: 0.05em;">Recommended Next Technologies to Learn</h6>
                        <div class="d-flex flex-wrap gap-2">
                            ${(data.recommended_technologies || []).map(tech => `
                                <a href="/recommendations?query=${encodeURIComponent(tech)}" class="badge badge-cyan-subtle p-2 small text-decoration-none fw-semibold" title="Explore opportunities with ${tech}">
            ...
            <div class="row g-4">
                <div class="col-lg-6">
                    <div class="card bg-surface-card p-4 shadow-md">
                        <h5 class="text-white fw-bold mb-3"><i class="bi bi-briefcase me-2 text-secondary"></i>Recommended Career Matches</h5>
                        <div class="list-group list-group-flush bg-transparent">
                            ${(data.recommended_roles || []).map(role => `
                                <div class="list-group-item bg-transparent border-subtle px-0 py-2.5 d-flex justify-content-between align-items-center">
                                    <span class="text-white fw-semibold"><i class="bi bi-chevron-right text-primary me-2"></i>${role}</span>
                                    <a href="/recommendations?query=${encodeURIComponent(role)}" class="btn btn-sm btn-glass px-2.5 small fw-bold">Find Openings &rarr;</a>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                ...
                <div class="col-lg-6">
                    <div class="card bg-surface-card p-4 shadow-md">
                        <h5 class="text-white fw-bold mb-3"><i class="bi bi-patch-check me-2 text-emerald"></i>Target Certifications</h5>
                        <div class="list-group list-group-flush bg-transparent">
                            ${(data.recommended_certifications || []).map(cert => `
                                <div class="list-group-item bg-transparent border-subtle px-0 py-2.5 d-flex justify-content-between align-items-center">
                                    <span class="text-white fw-semibold"><i class="bi bi-award text-emerald me-2"></i>${cert}</span>
                                    <a href="/recommendations?type=certification&query=${encodeURIComponent(cert)}" class="btn btn-sm btn-glass px-2.5 small text-emerald fw-bold">Explore Exam &rarr;</a>
                                </div>
                            `).join('')}
                        </div>
                    </div>
